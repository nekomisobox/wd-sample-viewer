...
どちらも bridge に送り、WD14 でタグ抽出 →（設定が ON なら）サンプル生成 → Chrome 取込ページに結果を蓄積します。
...
**対処の例:** 該当フレームをスクリーンショットする、または Chrome 取込ページ（`http://127.0.0.1:8777/`）へ画像をドラッグ＆ドロップ／貼り付けしてタグ抽出する。
